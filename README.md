...
`places` 시드 79곳은 [한국관광공사 TourAPI](https://www.data.go.kr)(KorService2)에서
가져왔습니다. 지역기반 목록(`areaBasedList2`) · 상세소개(`detailCommon2`) ·
영업시간(`detailIntro2`) 세 오퍼레이션을 조합해 이름·주소·좌표·사진·한 줄 소개·영업시간을
채웠습니다.
...
- `tags` — 전부 빈 배열. TASTE_TAGS 와 매칭할 신뢰할 만한 원천이 없어 추측해 채우지
  않았습니다.
- **술집(`sulzip`) 카테고리는 4개 지역 전부 비어 있습니다.** TourAPI 는 관광·가족 단위
  콘텐츠 위주라 주점 분류(FD04)가 전 지역 0건이고, "포차"·"펍" 키워드 검색도 결과가
  없었습니다. 지도에서 술집 필터를 눌러도 아무것도 뜨지 않습니다 — 버그가 아니라 데이터
  공백입니다. 카카오 로컬 API 등 다른 소스로 보충하기 전까지는 이 상태가 유지됩니다.

수집 스크립트는 저장소에 포함돼 있지 않습니다(1회성 배치 작업). 재수집이 필요하면
`supabase/migrations/20260825000000_tourapi_places.sql` 을 참고해 같은 방식으로 다시
만들 수 있습니다.
